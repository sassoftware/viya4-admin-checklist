...
The [SAS Viya platform's Programming Run-Time Environment](https://go.documentation.sas.com/doc/en/sasadmincdc/default/calsrvpgm/n01000viyaprgmsrvs00000admin.htm) [Doc] is the set of components which work together to run SAS programs. It is primarily made up of:
* A [SAS Launcher Service](https://go.documentation.sas.com/doc/en/sasadmincdc/default/calsrvpgm/n00002viyaprgmsrvs00000admin.htm) [Doc], that provides API endpoints for 'launching' (running) SAS Compute Server, SAS Connect Server and SAS Batch Server pods - the pods that contain the sas-programming-environment container, which run the processes that actually execute SAS programs
* A [SAS Compute Service](https://go.documentation.sas.com/doc/en/sasadmincdc/default/calsrvpgm/n00001viyaprgmsrvs00000admin.htm#n05002viyaprgmsrvs00000admin) [Doc] which provides API endpoints for requesting a SAS Compute Server session. It calls the SAS Launcher Service to ask it to start a SAS Compute Server pod, each of which runs one instance of the SAS Compute Server process. Once a SAS Compute Server pod has started up and registered itself with the SAS Compute Service, the SAS Compute Service will send code to it to be run, and will receive back execution status (pending, running, finished etc.), along with SAS log output, results etc. The SAS Compute Service can start (via SAS Workload Management and the SAS Launcher) as many SAS Compute Servers as it needs to, running at the same time. When SAS Compute Server processes finish, their pods usually terminate (unless they are configured to be re-usable). In contrast, only one instance of the SAS Compute Service usually runs at a time, and it remains running even when no SAS Compute sessions are currently active.
* A [SAS/CONNECT Spawner](https://go.documentation.sas.com/doc/en/sasadmincdc/default/calsrvpgm/n00005viyaprgmsrvs00000admin.htm) [Doc] which serves several functions, including providing API endpoints for requesting a launched SAS/CONNECT Server session. When these are used it uses SAS Workload Management and the SAS Launcher Service to start a SAS Connect Server pod, which runs for as long as the SAS Connect session lasts.
* A [SAS Batch Service](https://go.documentation.sas.com/doc/en/sasadmincdc/default/calsrvpgm/n0h9w6up3rf2a8n1j68cgh3u74f8.htm) [Doc] which calls SAS Workload Management and the SAS Launcher Service to launch SAS Batch Server pods which to process SAS programs in a batch mode, and which run for as long as each SAS Batch job lasts.
* [SAS Workload Management and SAS Workload Orchestrator](https://go.documentation.sas.com/doc/en/sasadmincdc/default/wrkldmgmt/p09rquinumlfumn1gwfza7sfdt30.htm) [Doc], enabled by default in recent releases of SAS Viya, which play a key role in scheduling pods on Kubernetes nodes for launched SAS programming run-time workload, taking into consideration queues, node pools, user process limits and resource capacity on each node. See [Servers That Can Use SAS Workload Orchestrator](https://go.documentation.sas.com/doc/en/sasadmincdc/default/wrkldmgmt/n1pxccy21jg4ytn1lb9kk8k38w9q.htm) [Doc]

> **Note**: This is one of the few topics where it is worth paying attention to the difference between the terms `service` and `server`:
>
...
SAS Launcher Contexts are specifications that apply environmental settings, define values for configuration attributes, and specify access constraints on launched Programming Run-Time pods. See SAS Viya Platform Administration > Servers and Services > Programming Run-Time Servers > SAS Launcher: Service and Contexts > SAS [Launcher Context](https://go.documentation.sas.com/doc/en/sasadmincdc/default/calsrvpgm/n092nfn11mpvmpn1eb4bh2vtpsgh.htm) [Doc] for a brief explanation.
...
Compute contexts provide a place to [specify attributes](https://go.documentation.sas.com/doc/en/sasadmincdc/default/calsrvpgm/n1jb3fo13t6cdbn16dv8dtfbox7v.htm#p0puuirx6357pyn19li1m698x9ky) [Doc] that modify the behaviour of a compute session. Examples include **runServerAs**, used to run sessions under the context as another user, **reuseServerProcesses** used to make compute servers reusable, or **serverMinAvailable**, used to pre-start a pool of available servers. There are many other attributes you can set.
...
Implementing a similar idea to Compute Server Contexts, by default there is only one Connect Context named default-launcher. It specifies an associated launcher context, SAS Workload Management queue (if applicable), and provides a place to specify SAS Options.

See [SAS/CONNECT Service Contexts](https://go.documentation.sas.com/doc/en/sasadmincdc/default/calsrvpgm/n05011viyaservers000000admin.htm#p1o2hz9al5f5uan19re8nyvgsvu0) [Doc] for more detail.

### SAS Batch Contexts

Again implementing a similar idea to SAS Compute Contexts, by default there are only two Batch Contexts, named default and default-cmd.

Each specifies an associated Launcher Context, and can specify a SAS Workload Orchestrator queue, and SAS options.

> **Note:** SAS Batch Contexts do not have autoexec code blocks. You can configure something similar using %include statements in your SAS batch programs to include commonly-needed SAS code.
...
* [Create group-specific and user-specific compute contexts – Part 1 – manual method](https://communities.sas.com/t5/SAS-Communities-Library/Create-group-specific-and-user-specific-compute-contexts-Part-1/ta-p/909069) [Blog]
* [Create group-specific and user-specific compute contexts – Part 2 – scripted method](https://communities.sas.com/t5/SAS-Communities-Library/Create-group-specific-and-user-specific-compute-contexts-Part-2/ta-p/910825) [Blog]
